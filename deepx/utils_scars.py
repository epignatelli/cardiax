...
def makePolyAndSplineCurve(r0=30, maxProtrudeFactor=0.3, NGon=11, NSplinePts=100):
    ...
    rperturbation = np.random.normal(
        loc=0.5, scale=0.6, size=NGon
    )  # random.randn(NGon)
    #
...
def GetAvgEdgeSize(SoftenedMask):
    ...
    for i in range(SoftenedMask.shape[0]):
        row_edge_sum = ((SoftenedMask[i] > 0) & (SoftenedMask[i] < 0.99)).sum()
        row_edge_mean = ((SoftenedMask[i] > 0) & (SoftenedMask[i] < 0.99)).mean()
...
def SoftenPolyAndSplineCurve(
    ...
    def ConvolveAndAvgEdge(LocalGaussShape, LocalGaussSigma):
        ...
        g2_a = makegauss2D(
            shape=(LocalGaussShape, LocalGaussShape), sigma=LocalGaussSigma
        )
...
def MakeAndSumCompositeBlob(params=def_params, CentroidSpline=None):
    ...
    for i in range(nBlobs):
        ...
        _, SplinePts = makePolyAndSplineCurve(
            r0=r0, maxProtrudeFactor=maxProtrudeFactor
        )
        ...
        if ADD_GAP & (i == nBlobs // 2):
            _, GapPts = makePolyAndSplineCurve(
                r0=r0 * GapFactor, maxProtrudeFactor=maxProtrudeFactor * GapFactor
            )
            for j in range(2):
    ...
    if CONSERVATIVE_CENTROID:
        GlobalCentroid = (
            (maxFieldX - doubleExtension) * np.random.rand() + maxExtension,
            (maxFieldY - doubleExtension) * np.random.rand() + maxExtension,
        )
    else:
        GlobalCentroid = (
            (maxFieldX - maxExtension) * np.random.rand() + halfExtension,
...
def save_scar_as_array(
    SoftenedComposite, params=def_params, root_file_name=def_root_file_name
):
    ...
    for content, ext in zip(
        ...
        if ext == "npy":
            ...
            imsave(
                formatted_file,
                np.round(SoftenedComposite * 255).astype(np.uint8),
                check_contrast=False,
            )


def load_scar_as_array(shortID=def_shortID, root_file_name=def_root_file_name):
    SoftenedComposite = np.load(
        root_file_name.format(content="output_scar", ID=shortID, ext="npy"),
        allow_pickle=False,
    )
    return SoftenedComposite


def random_diffusivity_scar(params: dict = def_params):
    ...
    while not valid_scar:
        try:
            ...
            print(
                "Attempt at generating random scar map failed because of an unexpected error. Trying again"
            )
            valid_scar = False
