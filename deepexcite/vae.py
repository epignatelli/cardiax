...
import utils
from utils import log, Elu, Downsample, Normalise, Flatten, Unflatten
import random
import numpy as np
...
#         self.propagator = BNAF(hidden_size, 5, hidden_size)
    ...
    def propagate(self, x):
        ...
        log("logvar", logvar.shape)
        z = self.reparameterise(mu, logvar)
        log("after flow", x.shape)
        x = self.unflatten(x)
...
#         flow_loss = self.propagator.compute_kl_pq_loss()
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.get_loss(y_hat[:, 2], x[:, 2])
        kld = self.kl_divergence(mu, logvar)
        tensorboard_logs = {**loss, **{"kdl": kld}}
        ...
        return {"loss": sum(loss.values()) + (kld * 0.001), "log": tensorboard_logs}
...
if __name__ == "__main__":
    ...
    model = VAE(args.channels, scale_factor=args.scale_factor,
    ...
    trainer = Trainer.from_argparse_args(parser, fast_dev_run=args.debug, row_log_interval=args.log_interval, default_root_dir="lightning_logs/vae")
