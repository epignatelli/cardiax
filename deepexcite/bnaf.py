...
class BNAF(nn.Module):
    ...
    def compute_kl_qp_loss(self, target_potential_fn, batch_size):
        ...
        q_log_prob = self.base_dist.log_prob(z)
        zk, logdet = self(z)
    ...
    def compute_kl_pq_loss(self, sample_2d_data_fn, batch_size):
        ...
        sample = sample_2d_data_fn(batch_size).to(self.base_dist.loc.device)
        z, logdet = self(sample)
